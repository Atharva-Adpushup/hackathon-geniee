...
module.exports = React.createClass({
    ...
    renderGuiderFinishMessage: function(){
        return (<Modal closeButton={false} onRequestHide={new Function} title="loading" className="_ap_modal_logo" keyboard={true} animation={true}>
            <div className="spin"></div>
            <div className="modal-body">
                <h4>All Done</h4>
                <Button style={{fontSize: 22}}  className="btn-lightBg"  onClick={this.onGlassClick}>Let's Start!</Button>
            </div>
        </Modal>);
    },
    componentDidMount:function(){
        ...
        setTimeout(function(){
            $("#miscMenuIntroNext").on("click",function(){
    ...
    saveTemplate:function(tpl){
        if (tpl.id) {
            ...
            this.getFlux().actions.createAction({
                ...
                tpl: this.getFlux().store(CommonConsts.enums.stores.TPL_STORE).getAdsenseTplByName(tpl.name)
            })
        }
        ...
        return (
            <div className="helpInsert">
                <Row>
                    ...
                    <Col xs={4}><i className="fa fa-arrows"></i></Col>
                </Row>
                <Row>
                    <Col xs={8}>
                        <h5 className="helpHead">Select Size</h5>
    ...
    render: function () {
        var colorAction = this.props.action.getActionByKey(availableActions.ADSENSE_COLORS),
            adNetworkActions =  this.getFlux().store("ActionsStore").getAdNetworkByOwner(levels.SITE);


        return (
            <Menu id="miscMenu" onGlassClick={this.onGlassClick} targetX={this.props.position.posX} targetY={this.props.position.posY} activeItem={this.state.activeMenuItem}>
                <MenuItem icon="fa fa-google" contentHeading="Adsense Configuration" contentComponent={<Settings noOfAds={this.props.adsense.maxAdsToDisplay} pubId={this.props.adsense.pubId} email={this.props.adsense.email} onSave={this.saveSettings}/>}/>
                <MenuItem icon="fa fa-eyedropper" contentHeading="Adsense Color Templates" contentComponent={<Templates flux={this.getFlux()} onSaveStatus={this.saveColorStatus} onNewTemplate={this.saveTemplate} action={colorAction} />}/>
                <MenuItem icon="fa fa-cog" contentHeading="AdTypes Configuration" contentComponent={<AdTypes onSave={this.saveAdtypes} adNetworkActions={adNetworkActions} />}/>
                <MenuItem icon="fa fa-retweet" contentHeading="Convert Control" contentComponent={<ControlConverter isApex={this.props.apex}/>} />
                <MenuItem icon="fa fa-cogs" contentHeading="Optimization Settings" contentComponent={<ApSettings isApex={this.props.apex} allChannels={this.props.allChannels} hideMenu={this.hideMenu} onSave={this.saveApSettings} apConfigs={this.props.apConfigs}/>} />
                <MenuItem icon="fa fa-sliders" contentHeading="Tools" contentComponent={<PluginPage flux={this.getFlux()}/>}/>
