...
module.exports = React.createClass({
    ...
    getAdcode: function () {
        ...
        if (this.props.isApex) {
            newAdCode = ['<ins class="adPushupAds" data-ver="2" data-siteId="' + ADP_SITE_ID + '" data-ac="' + encodedAd + '"><\/ins>' + '<script data-cfasync="false" type="text/javascript">(function (w, d) { for (var i = 0, j = d.getElementsByTagName("ins"), k = j[i]; i < j.length; k = j[++i]){ if(k.className == "adPushupAds" && k.getAttribute("data-push") != "1") { ((w.adpushup = w.adpushup || {}).control = (w.adpushup.control || [])).push(k); k.setAttribute("data-push", "1");} } })(window, document);</script>'];
        } else {
            newAdCode = [
                '<ins class="adPushupAds" data-ver="2" data-siteId="' + ADP_SITE_ID + '" data-ac="' + encodedAd + '"><\/ins>' +
    ...
    toogleTransform: function () {
        ...
        if(!this.isCodeAdsense()){
            ...
            !adCode ?this.setState({errorMessage:"There was some issue in control conversion plz contact support"}):this.setState({errorMessage:null,adCode:adCode, transForm: !this.state.transForm});
        }else{
    ...
    selectAll: function(ev){
        $(React.findDOMNode(this.refs.code)).find("textarea").select()
    },
    render: function () {
                ...
                {!this.state.transForm ?
                    <div>
                        {this.state.errorMessage ?
                            <Row>
                                <Col md={12} style={{color:"red"}} className="error"><b>{this.state.errorMessage}</b></Col>
                            </Row>
                                :null
                        }
                        <Row>
                            <Col md={12}><b>Paste Your Ad Code Here</b></Col>
                        </Row>
                        <Row>
                            <Col md={12}>
                                <Input className="txtareaFixedwidth" key="new" type='textarea' valueLink={this.linkState('adCode')}  placeholder='Paste your Adcode Here.' />
                            </Col>
                        </Row>
                        <Row className="butttonsRow">
                            <Col xs={12}>
                                <Button className="btn-lightBg btn-transform btn-block" onClick={this.toogleTransform}>Transform</Button>
                            </Col>
                        </Row>
                    </div>
                    :
                    <div>
                        <Row>
                            <Col md={12}><b>Transformed Code</b></Col>
                        </Row>
                        <Row>
                            <Col md={12}>
                                <Input className="txtareaFixedwidth" key="code" ref="code" type='textarea' onClick={this.selectAll} value={this.getAdcode()} />
                            </Col>
                        </Row>
                        <Row className="butttonsRow">
                            <Col xs={12}>
                                <Button className="btn-lightBg btn-save btn-block" onClick={this.transformAnother}>Transform Another</Button>
                            </Col>
                        </Row>
                    </div>
                }
