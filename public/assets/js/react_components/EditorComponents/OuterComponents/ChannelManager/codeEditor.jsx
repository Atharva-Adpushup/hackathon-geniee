...
module.exports = React.createClass({
    getInitialState: function () {
        //TODO: Send computed str as a prop from parent rendering component
        // and remove these below vendor specific str manipulation
        var vendorConfig = {
                "adRecover": {
                    "str": "adRecover.",
                    "regex": /adpushup./g
        ...
        defaultState = (this.props.isAdRecover) ? defaultState.replace(vendorConfig.adRecover.regex, vendorConfig.adRecover.str) : defaultState;
        defaultState = (this.props.isApex) ? defaultState.replace(vendorConfig.apex.regex, vendorConfig.apex.str) : defaultState;

        return {code: this.props.data.code ? atob(this.props.data.code) : (this.props.data.owner == "channel_incontent" ? defaultIncontentState: defaultState)};
    },
    ...
    save: function(){
        if(this.props.data.owner == "channel_incontent"){
            ...
            this.props.flux.actions.saveChannelBeforeAfterJs(this.props.data.id,this.props.data.type,btoa(this.state.code));
        }
    ...
    render: function () {
        ...
        return (<Modal className="_ap_modal" onRequestHide={this.close} keyboard={true} title='Code Editor' backdrop='static' animation={true}>
            {
                CodeMirror({
            ...
            <Row className="modal-footer modalfooter-btn codeEditorFooter">
                <Col xs={6}>
                    <Button ref="saveUrl" onClick={this.save} className="btn-lightBg btn-save">Save Code</Button>
                </Col>
                <Col xs={6}>
                    <Button onClick={this.close} className="btn-lightBg btn-cancel">Close</Button>
                </Col>
            </Row>
        </Modal>)
    }
