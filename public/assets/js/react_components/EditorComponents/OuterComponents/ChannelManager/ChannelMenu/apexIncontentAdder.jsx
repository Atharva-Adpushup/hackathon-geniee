...
module.exports = React.createClass({
	...
	getInitialState: function() {
		var apexIncontentSection = this.props.apexIncontentSection,
			...
			defaultCss = { float: 'none', clear: 'both', margin: '5px auto 5px auto'};

		return {
			manageAdBox: false,
			css: (isApexSection && apexIncontentSection.inContentSettings.css) ? apexIncontentSection.inContentSettings.css : defaultCss,
			secondaryCss: (isApexSection && apexIncontentSection.inContentSettings.secondaryCss) ? apexIncontentSection.inContentSettings.secondaryCss : null,
	...
	setCss: function(payload) {
		var css = {float: 'none', margin: '5px'},
			secondaryCss = {float: 'none', clear: 'both', margin: '5px auto 5px auto'};

		if (typeof payload === 'object') {
			this.setState({css: payload});
		} else if (payload === 'left') {
			this.setState({float: payload, secondaryCss: secondaryCss, css: Object.assign({}, css, {float: payload})});
		} else if (payload === 'right') {
			this.setState({float: payload, secondaryCss: secondaryCss, css: Object.assign({}, css, {float: payload})});
		} else if (payload === 'none') {
			this.setState({float: payload, secondaryCss: null,  css: Object.assign({}, css, {float: payload, clear: 'both', margin: '5px auto 5px auto'})});
		}
	},
	...
	getUpdatedSection: function() {
		...
		if (section) {
			...
			if (this.state.secondaryCss && Object.keys(this.state.secondaryCss).length > 0) {
				section.inContentSettings.secondaryCss = $.extend(true, {}, this.state.secondaryCss);
			} else {
				delete section.inContentSettings.secondaryCss;
			}
	...
	saveApexSettings: function() {
		...
		if (this.state.secondaryCss && Object.keys(this.state.secondaryCss).length > 0) {
			sectionObj.inContentSettings.secondaryCss = $.extend(true, {}, this.state.secondaryCss);
		}

		this.getFlux().actions.createApexIncontentSection(sectionObj);
	},
		...
		return (<div className="containerButtonBar sm-pad">
				<Row>
					<Col className="u-padding-b5px" xs={12} md={12}>
						<Button className="btn-lightBg btn-back" onClick={this.props.onUnloadCallback}>
							Go Back
						</Button>
					</Col>
				</Row>
