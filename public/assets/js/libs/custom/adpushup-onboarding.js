	...
	(function(ap, w, d) {
		...
		ap.onboarding = {
			...
			templates: {
				...
				dashboardLink:
					'<div class="text-center" style="margin-top: 15px;"><a style="font-size: 1.2em;" class="link-primary" href="/user/dashboard">Go to dashboard</a></div>',
				addOtherSiteFinish:
					'<div class="text-center"><h2>Thank you for the information. Our team will contact you soon.</h2></div>'
			},
			...
			generateInitCode: function(siteId) {
				...
				$('#header-code').text(
					'<script data-cfasync="false" type="text/javascript">' + headerCode + '</script>'
				);
			},
			...
			showStep: function(step) {
				...
				if (newSite.addedSite && step >= 2) {
					if (parseInt(newSite.addedSite.step) != 1) {
						this.manipulateElem(
							'#addSiteStr',
							'<h2 class="text-appear"><span>' +
								this.domanize(newSite.addedSite.domain) +
								'</span> has been Added!</h2>',
							'htmlFadeIn',
							600
						);
					}
				...
				$.post(
					...
					function(res) {
						if (res.success) {
							...
							ob.manipulateElem(
								'#addSiteStr',
								'<h2 class="text-appear"><span>' + site + '</span> has been Added!</h2>',
								'htmlFadeIn',
								600
							);
							...
							ob.manipulateElem(
								'#non-admin-email',
								'*Email* - ' + ob.extractWebsiteName(newSite.addedSite.domain) + '@adpushup.com',
								'htmlFadeIn',
								600
							);
							return true;
			...
			addSite: function(site, url, btn, flag) {
				...
				if (newSite.addOtherSite || flag) {
					...
					if (siteAlreadyAdded()) {
						ap.apAlert(
							ob.domanize(url) + ' has already been added! Please add a different site.',
							'#apdetect',
							'inverted',
							'slideDown'
						);
			...
			detectApSuccess: function(ob, el) {
				ap.apAlert(
					'AdPushup has been successfully detected on the website!',
					'#apdetect',
					'success',
					'slideDown'
				);
				...
				$.get(
					...
					function(res) {
						if (res.ap) {
							...
							ap.apAlert(
								'AdPushup was not detected on the website! Please make sure that the header code is present on the the specified URL',
								'#apdetect',
								'inverted',
								'slideDown'
							);
			...
			oauthHook: function(event) {
				...
				if (adsense.data) {
					...
					$('#adsenseoauth')
						.html('Google Adsense Connected ' + ap.onboarding.templates.checkIcon)
						.prop('disabled', true);
					$.post(
						'/user/setSiteStep',
						{
				...
				$.post('/user/setSiteServices', dataToEndPoint, function(response) {
					if (response.success) {
						if (!newSite.addOtherSite) {
							var status = 65;
							if (
								parseInt(window.currentUser.revenueUpperLimit) > 10000 ||
								data.selectedServices.length > 1
							) {
								status = 72;
							}
							ob.updateCrmDealStatus(status);
		...
		$('#addSiteAltForm').submit(function(e) {
			...
			(site = url.replace(/.*?:\/\//g, '')), (btn = $('#addSiteAltForm button'));
