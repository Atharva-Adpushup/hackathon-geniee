		...
		const deferred = $.Deferred(),
			processData = (rawData) => {
				...
				_.forOwn(computedResult.channelData.byIds, (chnlData, channelName) => {
					computedResult.channelData.byIds[channelName].variations = _.keys(computedResult.channelData.byIds[channelName].variations);
				});

				// Change 'sections' keys object to keys array
				_.forOwn(computedResult.variationByIds, (variationData, variationName) => {
					computedResult.variationByIds[variationName].sections = _.keys(computedResult.variationByIds[variationName].sections);
				});
