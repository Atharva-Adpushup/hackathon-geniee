...
module.exports = React.createClass({
    ...
    componentWillReceiveProps: function(nextprops) {
        if(Utils.deepDiffMapper.test(this.props.apConfigs,nextprops.apConfigs).isChanged && !this.state.manageBlockList){
    ...
    setValue: function (target,ev) {
        ...
        switch(target){
            case 'adpushupPercentage':
                if (val > 100)
                    this.setState({'adpushupPercentage':100})
                else if(val < 0)
                    this.setState({'adpushupPercentage':0})
                else
                    this.setState({'adpushupPercentage':val})
                break;
            case 'engineRequestTimeout':
                if(val < 1000)
                    this.setState({'engineRequestTimeout':1000})
                else
                    this.setState({'engineRequestTimeout':val})
                break;
            case 'xpathWaitTimeout':
                if(val < 1000)
                    this.setState({'xpathWaitTimeout':1000})
                else
                    this.setState({'xpathWaitTimeout':val})
                break;
        }
    },
    ...
    saveSettings: function(){
        this.props.onSave({
            displayMethod: (this.state.displayMethod ? CommonConsts.enums.site.displayMethod.ASYNC : CommonConsts.enums.site.displayMethod.SYNC),
        ...
        return (
            <Row>
                <Col className="u-padding-r10px" xs={12}>
                    <Button className="btn-lightBg btn-edit" onClick={this.toggleListManagers.bind(null, 'pageGroupPattern')}>Manage Page Group Pattern</Button>
                </Col>
            </Row>
        )
    },
    renderTrafficDistributionLayout: function() {
        return (
            <Row>
                <Col className="u-padding-r10px" xs={12}>
                    <Button className="btn-lightBg btn-edit" onClick={this.toggleListManagers.bind(null, 'trafficDistribution')}>Manage Traffic Distribution</Button>
                </Col>
            </Row>
        )
    },
    ...
    render: function () {
        if(this.state.manageBlockList){
           return (<BlockListManager renderInputText={true} blockList={this.props.apConfigs.blocklist || []} onSave={this.saveBlockList} onBack={this.toggleListManagers.bind(null, 'blockList')} />);
        }

        if (this.state.managePageGroupPattern) {
            var listArr = this.isAdRecoverInSite() ? this.props.apConfigs.adRecover.pageGroupPattern : (this.props.isApex ? (this.props.apConfigs.pageGroupPattern || []) : []);

            return (<TextRangeListManager list={listArr} onSave={this.savePageGroupPatternList} onBack={this.toggleListManagers.bind(null, 'pageGroupPattern')} />);
        }

        if (this.state.manageTrafficDistribution) {
            var collection = this.props.allChannels;

            return (<NumericCollectionManager savedCollection={this.props.apConfigs.trafficDistribution} collection={collection} onSave={this.saveTrafficDistributionConfig} onBack={this.toggleListManagers.bind(null, 'trafficDistribution')} />);
        }
            ...
            <div className="containerButtonBar sm-pad">
                ...
                <Row>
                    ...
                    <Col className="u-padding-r10px" xs={4}>
                        <input type='number' onChange={this.setValue.bind(null,'adpushupPercentage')} value={this.state.adpushupPercentage} />
                    </Col>
                </Row>
                <Row>
                    <Col className="u-padding-r10px" xs={8}>
                    ...
                    <Col className="u-padding-r10px" xs={4}>
                        <input type='number' onChange={this.setValue.bind(null,'engineRequestTimeout')} value={this.state.engineRequestTimeout} />
                    </Col>
                </Row>
                <Row>
                    <Col className="u-padding-r10px" xs={8}>
                    ...
                    <Col className="u-padding-r10px" xs={4}>
                        <input type='number' onChange={this.setValue.bind(null,'xpathWaitTimeout')} value={this.state.xpathWaitTimeout} />
                    </Col>
                </Row>
                ...
                <Row>
                    ...
                    <Col xs={4}>
                        <EnableDisableSwitch  size="s" id="displayType" onChange={this.toggleDisplayMethod} checked={this.state.displayMethod} on="Async" off="Sync"/>
                    </Col>
                </Row>
                <Row>
                    <Col className="u-padding-r10px" xs={8}>
                    ...
                    <Col xs={4}>
                        <EnableDisableSwitch  size="s" id="explicit Platform" onChange={this.toggleExplicitPlatform} checked={this.state.explicitPlatform} on="Yes" off="No"/>
                    </Col>
                </Row>
                <Row>
                    <Col className="u-padding-r10px" xs={12}>
                        <Button className="btn-lightBg btn-edit" onClick={this.toggleListManagers.bind(null, 'blockList')}>Manage BlockList</Button>
                    </Col>
                </Row>
                ...
                <Row className="butttonsRow">
                    <Col className="pd-10" xs={12}>
                        <Button disabled={!isChanged} className="btn-lightBg btn-save btn-block" onClick={this.saveSettings}>Save</Button>
                    </Col>
                </Row>
