...
function validateReportData(oneDayBeforeData, twoDaysBeforeData, siteData) {
	const isOneDayBeforeData = !!(
			oneDayBeforeData &&
			isObject(oneDayBeforeData) &&
			oneDayBeforeData.data &&
			isObject(oneDayBeforeData.data)
		),
		isOneDayBeforeDataRootLevelObject = !!(
			isOneDayBeforeData &&
			oneDayBeforeData.data.media &&
			Object.keys(oneDayBeforeData.data.media).length === KEYS.MEDIA &&
			oneDayBeforeData.data.pageGroups &&
			Object.keys(oneDayBeforeData.data.pageGroups).length >= KEYS.PAGEGROUPS
		),
		isTwoDaysBeforeData = !!(
			twoDaysBeforeData &&
			isObject(twoDaysBeforeData) &&
			twoDaysBeforeData.data &&
			isObject(twoDaysBeforeData.data)
		),
		isTwoDaysBeforeDataRootLevelObject = !!(
			isTwoDaysBeforeData &&
			twoDaysBeforeData.data.media &&
			Object.keys(twoDaysBeforeData.data.media).length === KEYS.MEDIA &&
			twoDaysBeforeData.data.pageGroups &&
			Object.keys(twoDaysBeforeData.data.pageGroups).length >= KEYS.PAGEGROUPS
		),
...
function generateMailContent(dataObject) {
	const inputDataObj = extend(true, {}, dataObject),
		...
		reportTemplateString = `${lodash(MAIL.CONTENT.COMMON)
			.replace('___odbPageViews___', oneDayBefore.pageViews)
			.replace('___odbRevenue___', oneDayBefore.revenue)
			.replace('___tdbPageViews___', twoDaysBefore.pageViews)
			.replace('___tdbRevenue___', twoDaysBefore.revenue)}`;

	if (isInDraftMode) {
		mailContentObj.header = lodash(MAIL.CONTENT.DRAFT.HEADER)
			.replace('___sitename___', siteData.domain)
			.replace('___siteId___', siteData.id);
		mailContentObj.content = `${lodash(MAIL.CONTENT.DRAFT.CONTENT).replace(
			'___sitename___',
			siteData.domain
		)} ${reportTemplateString}`;
	} else if (isDataInConsistency) {
		mailContentObj.header = lodash(MAIL.CONTENT.DATA_INCONSISTENT.HEADER)
			.replace('___sitename___', siteData.domain)
			.replace('___siteId___', siteData.id);
		mailContentObj.content = `${lodash(MAIL.CONTENT.DATA_INCONSISTENT.CONTENT).replace(
			'___sitename___',
			siteData.domain
		)} ${reportTemplateString}`;
	}
...
function initModule(site, resolve, reject) {
	const isGenieePartner = !!(
		site.get('partner') &&
		site.get('partner') === CC.partners.geniee.name &&
		site.get('genieeMediaId')
	);

	if (!isGenieePartner) {
		return reject(new Error(MESSAGES.SITE_INVALID));
	}
	...
	const isMode = !!(site.get('apConfigs') && site.get('apConfigs').mode),
		...
		oneDayBeforeDate = moment()
			.subtract(1, 'days')
			.format(DATE.FORMAT['y-m-d']),
		twoDaysBeforeDate = moment()
			.subtract(2, 'days')
			.format(DATE.FORMAT['y-m-d']),
