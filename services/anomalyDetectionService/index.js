...
const config = require('../../configs/config'),
    ...
    localBucket = couchbaseService(`couchbase://${config.couchBase.HOST}`, "apLocalBucket", config.couchBase.DEFAULT_BUCKET_PASSWORD);

const logger = require('../../helpers/globalBucketLogger');

const { ANAMOLY_PAGE_VIEW_IMPRESSION_XPATH_MISS, ANAMOLY_CPM, ANAMOLY_DETERMINED_MODE } = require('../../reports/default/adpTags/constants');

const weekStartDate = moment().subtract(8, 'days').format('YYYY-MM-DD'),
    weekEndDate = moment().subtract(2, 'days').format('YYYY-MM-DD'),
    yesterdayDate = moment().subtract(1, 'days').format('YYYY-MM-DD');

function getAnomalyThresholds() {
    return localBucket.getDoc("data::anomaly-thresholds")
        .then(function (obj) {
            return obj.value;
        }).catch(function (ex) {
...
function getDataSetFromSql(thresholds) {
    ...
    return Promise.join(sqlHelper.queryDB(pVIXConfig), sqlHelper.queryDB(cpmConfig), sqlHelper.queryDB(dmConfig), (PVIXARecords, cpmARecords, DMRecords) => Promise.resolve([PVIXARecords, cpmARecords, DMRecords]));
}

function prepareAnomalyList(dataSets) {

    let speratedPVIX = _.filter(_.map(dataSets[0], seperateEachPVIX), function (obj) { return !_.isEmpty(obj) });

    return filterApAppBucketSites([speratedPVIX, dataSets[1], dataSets[2]]).then(function (combinedAnomally) {
        return Promise.resolve(_.groupBy((combinedAnomally), function (obj) {
            return obj.siteId;
        }));
    });
}
...
function raiseSupportTickets(anomalyList) {
    ...
    let emailId = `udit.jindal@adpushup.com`;
    let cc = "";
    ...
    _.forEach(anomalyList, function (siteAnomalyArray, siteId) {
        ...
        _.forEach(siteAnomalyArray, function (value, key) {
            ...
            rowContent += tableRowTemplate.replace(/@__reason__|@__oldValue__|@__newValue__|@__diff__/gi, function (matched) {
...
function writeExecutionLog(executionLog) {
    logger({
        ...
        message: 'Successfull Execution on ' + moment().utcOffset("+05:30").format('MMMM Do YYYY, h:mm:ss a'),
...
function filterApAppBucketSites(dataSets) {
    ...
    let query = couchbasePromise.ViewQuery.from('AdpTagReporting', 'SiteUserMapping').stale(1).reduce(false);
