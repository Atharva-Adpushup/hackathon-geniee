...
var getImmediateParentData = function(element) {
	...
	if (isValidInlineStyles) {
		...
		finalComputedStyles = adp.$.extend({}, computedStylesObject);
	}
...
var getFilteredData = function(inputData) {
	var windowWidth = adp.$(window).width(),
...
var getComputedAdSizes = function(adpSlot) {
	...
	if (isValidImmediateParentData) {
		computedParentData = adp.$.extend({}, immediateParentData);
	} else {
		computedParentData = adp.$.extend({}, recursiveParentData);
