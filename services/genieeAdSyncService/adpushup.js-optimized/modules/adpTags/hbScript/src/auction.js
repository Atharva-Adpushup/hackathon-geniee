...
var auction = {
	...
	requestBids: function(pbjs, adpBatchId, slotCodes, timeOut) {
		...
		pbjs.requestBids({
			...
			bidsBackHandler: () => {
				setTimeout(that.getAuctionResponse.bind(that, adpBatchId));
			}
		});
			...
			if (bidders.hasOwnProperty(bidderCode)) {
				bidderSettings[bidderCode] = {
					bidCpmAdjustment: function(bidCpm) {
						...
						if (this.bidType === 'gross' && !isNaN(this.revenueShare)) {
					...
					}.bind({ revenueShare: revenueShare, bidType:bidders[bidderCode].bids })
				};
			}
		}
