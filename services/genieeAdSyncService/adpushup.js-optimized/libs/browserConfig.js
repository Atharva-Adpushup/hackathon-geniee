var adp = window.adpushup,
	...
	Worker = require('./index.worker.js'),
	browserConfig = {
		name: 'other',
		platform: 'DESKTOP',
		detectPlatform,
...
function detectPlatform() {
	return new Promise((resolve, reject) => {
		if (window.Worker /*  && adpushup.utils.getQueryParams().enableworker */) {
			...
			worker.onmessage = function({ data: platform }) {
				browserConfig.platform = platform;
				resolve(browserConfig.platform);
			};
...
(function detectVisibilitySupport() {
	...
	pV.visibilitychangeEventName = vendorPrefix
		? vendorPrefix + 'visibilitychange'
		: 'visibilitychange';
})();
