...
module.exports = {
	log: function() {
		...
		var isQueryParams = !!(
				queryParams &&
				adp.$.isPlainObject(queryParams) &&
				!adp.$.isEmptyObject(queryParams)
			),
	...
	getCountry: function() {
		return new Promise(function(resolve, reject) {
			adp.$.get('//e3.adpushup.com/IpLocationPublicWebService/GetLocationInfo', function(response) {
	...
	requestServer: function (url, data, timeout, method, dataType, contentType) {
		...
		// adp.$.support.cors to be set back to false?
	},
	...
	getObjectByName: function(collection, name) {
		...
		if (collection && collection.length && name) {
			adp.$.each(collection, function(idx, obj) {
				if (name === obj.name) {
					isInCollection = true;
					objectConfig.index = idx;
					objectConfig.obj = adp.$.extend(true, {}, obj);
				}
			});
	...
	getScreenSize: function() {
		return [adp.$(window).width(), adp.$(window).height()];
	},
	...
	// 	}
	// },
	injectHeadCodeOnPage: function(src, onLoadHandler) {
		const scriptEl = document.createElement('script');
		scriptEl.type = 'text/javascript';
		scriptEl.async = true;

		if (onLoadHandler) scriptEl.onload = onLoadHandler;
	...
	injectJqueryIfDoesntExist: function(callback) {
		function compatibleJqueryExists() {
			if (window.$ && window.$.fn) {
				let [majorVersion] = window.$.fn.jquery.split('.');
				majorVersion = parseInt(majorVersion, 10);
				// $ exists and version of $ is 2.
				if (majorVersion === 2) {
					adp.$ = window.$;
					return true;
				}
