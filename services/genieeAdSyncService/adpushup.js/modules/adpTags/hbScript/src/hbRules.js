	...
	getComputedActions(hbRules) {
		...
		hbRules.forEach(hbRule => {
			for (var i = 0; i < hbRule.actions.length; i++) {
				...
				var oldActionDate =
					actionsMapping[action.key] && new Date(actionsMapping[action.key].createdAt);
		...
		var computedActions = Object.keys(actionsMapping).map(key => actionsMapping[key].action);
