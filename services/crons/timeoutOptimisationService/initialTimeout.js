			...
			for (const [timeout, timeoutConfig] of Object.entries(finalData)) {
				for (const [device, deviceConfig] of Object.entries(timeoutConfig)) {
					...
					rulesArray.push({
						...
						description: 'Initial Timeout Optimisation rules',
						isActive: true,
						triggers: [
			...
			try {
				...
				if (existingAutoRules.length) {
					existingAutoRules = existingAutoRules.filter(
						rule => rule.description != 'Initial Timeout Optimisation rules'
					);
				}
				...
				const updatedTimeoutRules = await updatedSiteHbConfig
					.get('autoRules')
					.filter(rule => rule.description == 'Initial Timeout Optimisation rules');
				...
				const isRuleCorrect =
					Math.round((totalRuleRevenue * 100) / totalSiteWiseRevenue[site]) < 10;
				if (!isRuleCorrect) {
					const revLost = Math.round((totalRuleRevenue * 100) / totalSiteWiseRevenue[site]);
					console.log(`Revenue lost is ${revLost}`);
