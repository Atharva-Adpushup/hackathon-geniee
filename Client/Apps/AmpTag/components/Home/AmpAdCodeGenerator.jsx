...
class AmpAdCodeGenerator extends Component {
	constructor(props) {
		super(props);
		this.state = {
			progress: 0,
	...
	saveHandler() {
		...
		const { createAmpAd, siteId } = this.props;
	...
	resetHandler() {
		const { resetCurrentAdAmp, siteId } = this.props;
		resetCurrentAdAmp(siteId);
		this.setState({
			progress: 0,
			platform: 'mobile',
			type: '',
...
export default AmpAdCodeGenerator;
