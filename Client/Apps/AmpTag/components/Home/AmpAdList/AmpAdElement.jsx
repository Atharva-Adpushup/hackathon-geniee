...
import { DISPLAYADCODE_AMP, STICKYADCODE_AMP } from '../../../configs/commonConsts';
...
class AdElement extends Component {
	...
	disableAd() {
		...
		if (window.confirm(message)) {
			this.setState(
				...
				() => this.updateWrapper({ ...ad, isActive: !isActive, archivedOn: +new Date() })
			);
		}
	}
	...
	updateWrapper(data) {
		const {
			...
			ad,
			siteId,
			adsToUpdateOnMasterSave,
			dataForAuditLogs
		} = this.props;
		adsToUpdateOnMasterSave(ad.id);
		return user.isSuperUser
			? updateAmpAd(ad.id, siteId, data)
			: modifyAmpAdOnServer(siteId, ad.id, data, dataForAuditLogs);
	}
