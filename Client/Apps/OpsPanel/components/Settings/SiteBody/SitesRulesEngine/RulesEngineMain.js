...
import RulesEngineActions from './RulesEngineActions';
import RulesEngineTriggers from './RulesEngineTriggers';
...
const getResetValueForAction = actionKey => {
	const arrayTypes = ['refreshType'];
	if (arrayTypes.includes(actionKey)) return [];
	return null;
};

class RulesEngineMain extends React.Component {
	constructor(props) {
		super(props);
		...
		this.state = {
			...
			triggers: [],
			actions: [],
			actionKeyOptions: RULES_ENGINE.RULES_ENGINE_ACTIONS_KEY_OPTIONS,
			actionValueOptions: RULES_ENGINE.RULES_ENGINE_ACTIONS_VALUE_OPTIONS,
			triggerKeyOptions: RULES_ENGINE.RULES_ENGINE_TRIGGER_KEY_OPTIONS,
	...
	handleSubmit() {
		...
		dataForAuditLogs = {
			...dataForAuditLogs,
			appName: 'Ops Panel - Site Settings'
		};
		...
		saveSiteRulesEngine(siteId, { rule, ruleIndex: selectedRuleIndex }, dataForAuditLogs)
			.then(() => {
				const notification = {
					mode: 'success',
	...
	handleToggleStatus(index, value) {
		...
		saveSiteRulesEngine(siteId, { rule, ruleIndex: index }, dataForAuditLogs)
			.then(() => {
				const notification = {
					mode: 'success',
					title: 'Operation Successful',
...
export default RulesEngineMain;
