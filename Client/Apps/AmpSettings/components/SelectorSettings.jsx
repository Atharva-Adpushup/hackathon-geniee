...
class SelectorSettings extends React.Component {
	...
	renderSelector = key => {
		const selectorConf = commonConsts.pagegroupSelectors;
	...
	renderArraySelector = (key, index) => {
		const selectorValue =
			(this.state.selectors[key][index] && this.state.selectors[key][index].value) || '';
		return (
	...
	render() {
		const selectorConf = commonConsts.pagegroupSelectors;
		return (
			<CollapsePanel title="Selector Setting" className="mediumFontSize" noBorder>
