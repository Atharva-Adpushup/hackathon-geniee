...
const AmpActiveBidderAdaptersList = model.extend(function() {
	...
	this.constructor = function(data, cas) {
		if (!data.activeBiddersInAscOrder || !data.prebidBundleName) {
			throw new Error(
				'activeBidders list and prebidBundleName are required for AmpActiveBidderAdaptersList doc'
			);
		}
		this.key = docKeys.ampActiveBidderAdaptersList;
		this.super(data, !!cas);
...
function apiModule() {
	const API = {
		getActiveBidderAdaptersList: function() {
			return couchbase
				.connectToAppBucket()
				.then(appBucket => appBucket.getAsync(docKeys.ampActiveBidderAdaptersList))
				.then(json => new AmpActiveBidderAdaptersList(json.value, json.cas));
		},
		createActiveBidderAdaptersList(activeBidders) {
			...
			return Promise.resolve(new AmpActiveBidderAdaptersList(json)).then(activeBidderAdaptersList =>
		...
		getActiveAndUsedBidderAdapters: function() {
			const queryString = commonConsts.PREBID_BUNDLING.AMP_ACTIVE_BIDDER_ADAPTERS_N1QL;
