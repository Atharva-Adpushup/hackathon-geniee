var AdPushupError = require('../helpers/AdPushupError'),
	...
	prepareApexSearchQuery = function(config) {
		var initialStartDate = moment().subtract(7, 'days').startOf('day').valueOf(),
			initialEndDate = moment().subtract(0, 'days').endOf('day').valueOf(),
			siteId = config.siteId,
			variationCount = config.variationCount ? config.variationCount : 100,
			startDate = (config.startDate) ? moment(config.startDate, 'x').startOf('day').valueOf() : initialStartDate,
			endDate = config.endDate ? moment(config.endDate, 'x').endOf('day').valueOf() : initialEndDate,
			pageGroup = config.pageGroup,
			platform = config.platform,
			b = esqm.createBoolFilter(),
			rangeFilter = esqm.createRangeFilter('createdTs', startDate, endDate),
