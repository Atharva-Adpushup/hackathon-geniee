...
function syncCDN(decodedMessage, originalMessage) {
	if (!decodedMessage.siteId) {
		throw new CustomError({
			message: 'Unable to sync file with cdn',
		 ...
		 */
			.catch(error => {
				...
				const isEmptyConsumerMessage = !!(
					customErrorMessage &&
					customErrorMessage.message === CONSTANTS.ERROR_MESSAGES.RABBITMQ.CONSUMER.EMPTY_MESSAGE &&
					!Object.keys(customErrorMessage.data).length
				);
				const isInvalidConsumerMessage = !!(
					customErrorMessage && customErrorMessage.message === CONSTANTS.ERROR_MESSAGES.MESSAGE.INVALID_DATA
				);
				...
				if (isEmptyConsumerMessage || isInvalidConsumerMessage) {
					...
					customErrorMessage.message === CONSTANTS.ERROR_MESSAGES.MESSAGE.CDN_SYNC_ERROR ||
					customErrorMessage.message === ERROR_MESSAGES.MESSAGE.UNSYNCED_SETUP
				) {
					console.log(
						`Geniee Sync error : ${customErrorMessage.message} for site id : ${customErrorMessage.siteId}`
					);
