var consts = require('../configs/commonConsts'),
	...
	model = Class.extend(function() {
		...
		this.save = function() {
			...
			return couchbase.connectToAppBucket().then(function(appBucket) {
				return !self.casValue
					? appBucket.insertAsync(self.key, self.toJSON(), {}).then(function(obj) {
		...
		this.mergeObjectViaClassMap = function(existingData = {}, newData = {}, schema) {
			...
			_.forEach(unionKeys, function(unionKey) {
				var newDataObj = newData[unionKey],
					...
					isExistingDataAnObject = !!(
						existingDataObj &&
						_.isObject(existingDataObj) &&
						_.isPlainObject(existingDataObj)
					),
					isExistingDataAnArray = !!(
						existingDataObj &&
						_.isObject(existingDataObj) &&
						_.isArray(existingDataObj)
					),
		...
		this.mergeObjectsRecursively = function(existingData, newData, schema) {
			...
			_.forEach(intersectedKeys, function(intersectedKey) {
				var existingDataObj = existingData[intersectedKey],
					...
					isSchema,
					isSchemaKeyHasProperty,
					isSchemaClassMapHasProperty,
					isClassMapHasProperty,
					isPropertySupportDeepExtend,
					isPropertyNonExistent;
				...
				_.forEach(allIteratableKeys, function(propertyKey) {
					...
					isPropertySupportDeepExtend = !!(
						isSchemaKeyHasProperty && self.mergeObjectViaDeepExtendArr.indexOf(propertyKey) > -1
					);
					...
					if (isPropertySupportDeepExtend) {
						computedData[intersectedKey][propertyKey] = self.extendObject(
							existingDataObj[propertyKey],
							newDataObj[propertyKey]
						);
					} else if (isClassMapHasProperty) {
						computedData[intersectedKey][propertyKey] = self.mergeObjectViaClassMap(
							existingDataObj[propertyKey],
							newDataObj[propertyKey],
							self.classMap[propertyKey]
						);
					} else if (isSchemaClassMapHasProperty) {
						computedData[intersectedKey][propertyKey] = self.mergeObjectsRecursively(
							existingDataObj[propertyKey],
							newDataObj[propertyKey],
							schema.classMap[propertyKey]
						);
	 ...
	 */
		...
		this.mergeArrayModel = function(existingArr, newArr) {
			...
			_.forEach(secondArr, function(second) {
				// loop over second array
				var flag = false;
				_.forEach(firstArr, function(first) {
					...
					if (second.get(first.mergingKey) === first.get(first.mergingKey)) {
						...
						if (first.mergeExtraKeys) {
							_.forEach(_.difference(Object.keys(second.data), first.keys), function(key) {
								// find difference between allowed keys and keys of existing data
		...
		this.set = function(key, val, forceInsert) {
			var existingData = this.get(key),
				...
				isKeyAnArrayModel = !!(
					...
					existingData[0] &&
					existingData[0] instanceof model
				),
				isKeyMergedViaClassMap = !!(
					isExistingDataAnObject &&
					key &&
					this.mergeObjectViaClassMapArr.indexOf(key) > -1 &&
					this.classMap[key]
				),
				isKeyMergedViaDeepExtend = !!(
					isExistingDataAnObject && this.mergeObjectViaDeepExtendArr.indexOf(key) > -1
				);
