...
const dbHelper = couchbaseService(`couchbase://${config.globalBucket.HOST}`, config.globalBucket.DEFAULT_BUCKET, config.globalBucket.DEFAULT_BUCKET_PASSWORD);

function logger(logData) {
	return dbHelper.createDoc(`slog::${uuid.v4()}`, {
		date: +new Date(),
