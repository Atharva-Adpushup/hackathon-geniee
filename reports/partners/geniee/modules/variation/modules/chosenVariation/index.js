...
module.exports = {
	...
	getData: function(config, sqlReportData, variation, pageGroup) {
		...
		const computedData = {
			...
			isValidSiteId = !!(
				isSqlReportData &&
				sqlReportData.hasOwnProperty(computedData.siteId) &&
				sqlReportData[computedData.siteId]
			),
			isValidChannel = !!(
				isValidSiteId &&
				sqlReportData[computedData.siteId].pageGroups &&
				sqlReportData[computedData.siteId].pageGroups[computedData.channelName]
			),
			isValidVariationId = !!(
				isValidChannel &&
				sqlReportData[computedData.siteId].pageGroups[computedData.channelName].variations &&
				sqlReportData[computedData.siteId].pageGroups[computedData.channelName].variations[
					computedData.variationKey
				]
			);

		if (!isSqlReportData || !isValidVariationId) {
			return selectedVariationObj;
		}

		selectedVariationObj = extend(
			true,
			{},
			sqlReportData[computedData.siteId].pageGroups[computedData.channelName].variations[
				computedData.variationKey
