...
module.exports = {
	getReportData: function(reportConfig) {
		...
		function generateRPMReport(ctrPerformanceConfig, channel, email, tableFormatReportData, variationData) {
			return Promise.all(
				_.map(variationData, (variationObj, variationKey) => {
					...
					return pageGroupVariationRPMService
						.getReportData(variationRPMConfig, email, tableFormatReportData)
		...
		function isChannelDataInReport(channelName, pageGroupsObject) {
			const isChannelData = !!(pageGroupsObject.hasOwnProperty(channelName) && pageGroupsObject[channelName]),
				...
				isVariationData = !!(
					isVariation &&
					_.isObject(pageGroupsObject[channelName].variations) &&
					_.keys(pageGroupsObject[channelName].variations).length
				);
			...
			return Promise.all(
				_.compact(
					_.map(allChannels, channel => {
						...
						return ctrPerformanceService
							.getReportData(ctrPerformanceConfig, sqlReportData)
							.spread((updatedReportData, tableFormatReportData) => {
								return Promise.resolve(variationModule.getMetrics(updatedReportData)).then(
									generateRPMReport.bind(
										null,
										ctrPerformanceConfig,
										channel,
										email,
		...
		return Promise.join(getSqlReportData, getSiteModel, (sqlReportData, siteModelInstance) => {
			...
			return Promise.resolve(siteModelInstance).then(site => {
				const email = site.get('ownerEmail');

				return site.getAllChannels().then(generateFullReport.bind(null, config, email, sqlReportData));
			});
		});
	}
};
