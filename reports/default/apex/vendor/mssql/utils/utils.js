...
const getSqlValidParameterDates = function(params) {
	...
	var fromDate = '2017-07-26',
		...
		isInputFromDateInRange = !!(
			moment(fromDate).isBefore(inputFromDate) && moment(todayDate).isAfter(inputFromDate)
		),
		isInputFromDateEqualsAnyDateRange = !!(
			moment(fromDate).isSame(inputFromDate) || moment(todayDate).isSame(inputFromDate)
		),
		...
		isInputToDateComparison = !!(
			moment(inputToDate).isAfter(inputFromDate) || moment(inputToDate).isSame(inputFromDate)
		),
		isValidInputToDate = !!(
			(isInputToDateInRange && isInputToDateComparison) ||
			(isInputToDateComparison && isInputToDateEqualsFromDate) ||
			(isInputToDateComparison && isInputToDateEqualsToDate)
		);
