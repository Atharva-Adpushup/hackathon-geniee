var extend = require('extend'),
	...
	adCodeSlots = {
		get: function(config, site) {
			...
			if (isChannelPresent) {
				return channelModel
					.getChannel(config.siteId, config.platform, config.pageGroup)
		...
		extract: function(config, channel) {
			...
			adCodeSlotArr = lodash.uniq(
				lodash.compact(
					lodash.map(adCodeArr, function(adCode) {
						...
						if (isAdSlotPresent) {
							adCodeSlot = base64DecodedAdCode
								.match(/data-ad-slot="\d*"/)[0]
								.replace('data-ad-slot="', '')
								.replace('"', '');
							return adCodeSlot;
						}
