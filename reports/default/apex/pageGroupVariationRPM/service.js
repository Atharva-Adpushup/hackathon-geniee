...
module.exports = {
	getReportData: function(params, email, tableFormatReportData) {
		...
		return siteModel
			.getSiteById(queryConfig.siteId)
			...
			.then(function(adSlotsArr) {
				...
				return adsenseModule.getData(queryConfig, adSlotsArr).then(function(adsenseData) {
					return pageViewsModule.getTotalCount(variationId, tableFormatReportData).then(function(pageViews) {
						return rpmModule.calculate(pageViews, adsenseData.earnings).then(function(rpm) {
							return {
								success: true,
								rpm: rpm,
								pageViews: pageViews,
