...
class ReportControls extends Component {
	...
	pageGroupUpdated(pageGroup) {
		const { platform, startDate, endDate, variation, groupBy, tagManager } = this.state;
	...
	platformUpdated(platform) {
		const { pageGroup, startDate, endDate, variation, groupBy, tagManager } = this.state;
		let groupByParam = groupBy;
		if (tagManager) {
			if (platform !== null) {
				groupByParam = null;
	...
	variationUpdated(variation) {
		const { platform, pageGroup, startDate, endDate, groupBy, tagManager } = this.state;
	...
	groupByUpdated(groupBy) {
		const { pageGroup, platform, variation, startDate, endDate, tagManager } = this.state;
	...
	render() {
		const { state, props } = this,
			{ PLATFORMS, PAGEGROUPS, REPORT_DOWNLOAD_ENDPOINT, IS_MANUAL } = commonConsts,
					...
					<Row className="mT-10">
						...
						<Col sm={2}>
							...
							<SelectBox
								...
								disabled={!props.variations || !props.variations.length || this.state.tagManager}
							>
