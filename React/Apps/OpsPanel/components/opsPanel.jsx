...
import { getSupportedAdSizes, getActivePartners, getSizeWiseSetupGroups, loadHbConfigToPanel, removeOptionsIndex } from '../lib/helpers.js';
import '../styles.scss';
...
class OpsPanel extends React.Component {
    ...
    saveHbConfig() {
        ...
        if(state.hbConfig) {
            $.post(`/user/site/${window.siteId}/opsPanel/hbConfig`, {
                data: JSON.stringify(payload)
            }, (res) => {
                this.setState({ updateMessage: res.message });
                setTimeout(() => { this.setState({ updateMessage: '' }) }, 3000);
            });
        }
                            ...
                            <Row>
                                <Col sm={12}>
                                    <p className="hb-settings-text">
                                        This config will save information related to the partner parameters required in the header bidding setup.
                                    </p>
                                    {
                                        state.loading ? <div className="error-message">Loading...</div> : ''
                                    }
                                    <div className="hb-options-wrapper">
                                        <HbConfigCreator updateGlobalHbConfig={this.updateGlobalHbConfig} hbConfig={loadHbConfigToPanel(state.hbConfig)} partners={getActivePartners()} adSizes=
                                        {getSupportedAdSizes()} saveHbConfigCallback={this.fetchHbConfig}></HbConfigCreator>
                                    </div>
                                </Col>
                                <Col sm={4}>
                                    <div className="error-message">{state.updateMessage}</div>
                                    <button className="btn btn-lightBg btn-default" onClick={this.saveHbConfig}>Save setup</button>
                                </Col>
                            </Row>
                        </Col>
                    </Row>
