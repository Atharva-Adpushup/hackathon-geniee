...
const getAllVariations = (state) => state.variationByIds,
	...
	getActiveChannelActiveVariationWithAds = createSelector([getActiveChannelVariationsWithAds, getActiveChannelActiveVariationId],
	...
	getCustomAdCodeFromActiveVariation = createSelector([getActiveChannelActiveVariationWithAds], (activeVariationWithAds = {}) => {
		let isCustomAdCode = false;
		const isActiveVariation = !!(activeVariationWithAds && _.isObject(activeVariationWithAds) && _.keys(activeVariationWithAds).length);
		...
		if (isActiveVariation) {
			_.forEach(activeVariationWithAds.sections, (sectionObj) => {
				_.forEach(sectionObj.ads, (adObj) => {
					...
					if (isCustomAdCode) { return false; }
				});
...
export {
	...
	getChannelVariationsWithAds, getActiveChannelVariationsTrafficDistributions,
	getChannelVariations, getVariationStructuredSectionsWithAds, getActiveChannelActiveVariationWithAds,
