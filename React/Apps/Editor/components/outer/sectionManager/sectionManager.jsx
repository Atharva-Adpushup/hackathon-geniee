...
module.exports = React.createClass({
    ...
    toggleStatsMode: function () {
        if(!this.state.statsMode && ((this.props.channel.statsStatus == Status.NOT_LOADED) || (this.props.channel.statsStatus == Status.FAILED))){
            this.props.flux.actions.loadChannelStats(this.props.channel.id,this.props.channel.pageGroup,this.props.channel.platform);
        }
    ...
    renderFailMessage: function () {
        return (
                <div  className="message">
                    Oops! error while loading stats.
                </div>
        );
    },
            ...
            return (
                ...
                <ul className="list">
                    ...
                    <div className="statsWrapper">
                        <ul>
                        {
                            _(this.props.sections).map(function (section) {
                                if(section.stats)
                                    return (<SectionStats style={section.xpathMissing ? {backgroundColor:"red"} : null} onMouseOver={this.handleMouseOver.bind(this,section)} name={section.name} data={section.stats} />)
                            }.bind(this))
                        }
                        </ul>
                    </div>
              ...
              <li>
                <div className="statsWrapper">
                  <ul>
                    {
                      _(this.props.sections).map(function (section) {
                          return (<li className="level-og" key={section.id} style={section.xpathMissing ? {backgroundColor:"red"} :null} onMouseOver={this.handleMouseOver.bind(this,section)}>
                                    <div className="row">
    ...
    render: function () {
        if (!this.props.sections || !Array.isArray(this.props.sections) || !this.props.sections.length)
            return (null);
        ...
        return (
            <div className={!this.state.active ? "switcher-hidden" : null}>
                <div className={this.state.statsMode ? "sectionManager statsmode" : "sectionManager"}>
                  <a onClick={this.toggleSectionManager} className="switcher-toggle" title="Toggle the switcher">Toggle</a>
                  <div className="wrapper">
                    <div  className="heading">
                      <a>Section Manager</a>
                      <EnableDisableSwitch  size="s" id={"statsMode"} onChange={this.toggleStatsMode} checked={this.state.statsMode} on="Stats" off="Nav"/>
                    </div>
                    <div className="content">
                    {
                      !this.state.active ? this.renderCollapsed() : (this.state.statsMode ? this.renderWithStats() : this.renderWithoutStats())
                    }
                    </div>
                  </div>
                </div>
