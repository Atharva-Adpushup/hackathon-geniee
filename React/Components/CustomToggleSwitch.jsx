...
class customToggleSwitch extends React.Component {
	...
	renderVerticalLayout(options) {
		const labelClassNames = options.defaultLayout ? 'control-label' : 'u-padding-b5px';

		return (
			<div className={options.errorClassName}>
				<label className={labelClassNames}>{this.props.labelText}</label>
				{this.renderToggleSwitch()}
			</div>
		);
	}
	...
	render() {
		...
		return (
			<div key={this.props.name} className={options.layoutClassName}>
